...
-- Show only AGE, JOB, and BALANCE for all customers --
SELECT AGE,JOB,BALANCE FROM BANK_DETAILS;
-- Find all customers whose JOB is 'TECHNICIAN'--
SELECT * FROM BANK_DETAILS WHERE JOB="TECHNICIAN";
-- Find all customers who are SINGLE--
SELECT * FROM BANK_DETAILS WHERE MARITAL="SINGLE";
-- Show customers with BALANCE greater than 1000--
SELECT * FROM BANK_DETAILS WHERE BALANCE>1000;
-- Find customers who have a negative BALANCE--
SELECT * FROM BANK_DETAILS WHERE BALANCE<0;
-- Show all customers where HOUSING = 'YES' and LOAN = 'YES--
SELECT * FROM BANK_DETAILS WHERE HOUSING="YES" AND LOAN="YES";
-- Display customers contacted in the month of MAY--
SELECT * FROM BANK_DETAILS WHERE MONTH="MAY";
-- Show only the first 10 rows using LIMIT--
SELECT * FROM BANK_DETAILS LIMIT 10;
-- Display all distinct JOB values--
SELECT DISTINCT JOB FROM BANK_DETAILS;
-- Display all distinct EDUCATION values--
...
-- Show the top 5 highest balances (include AGE, JOB, BALANCE)--
SELECT AGE,JOB,BALANCE FROM BANK_DETAILS ORDER BY BALANCE DESC LIMIT 5;
-- Count the total number of customers--
SELECT COUNT(*) AS TOTAL_CUSTOMERS FROM BANK_DETAILS;
-- Find the average AGE of customers--
SELECT avg(AGE) FROM BANK_DETAILS;
-- Find the average AGE of customers rounded up value--
SELECT ROUND(avg(AGE),0) FROM BANK_DETAILS;
-- Find the maximum and minimum BALANCE --
SELECT min(AGE) AS "Minimum AGE", max(AGE) AS "Maximum Age" FROM BANK_DETAILS;
-- Find the total BALANCE of all customers --
SELECT sum(Balance) AS 'TOTAL BALANCE' FROM BANK_DETAILS;
-- Count how many customers subscribed (y = 'YES') --
SELECT count(*) FROM BANK_DETAILS  WHERE y="YES";
-- Count customers for each JOB --
SELECT JOB,count(*) AS "No of Customers" FROM BANK_DETAILS GROUP BY JOB;
-- Count customers for each MARITAL status --
SELECT MARITAL,count(*) AS "No of Customers" FROM BANK_DETAILS GROUP BY MARITAL;
-- Find the average BALANCE for each JOB --
SELECT JOB,AVG(BALANCE) FROM BANK_DETAILS GROUP BY JOB;
-- Find the highest BALANCE within each JOB --
SELECT JOB,max(BALANCE) FROM BANK_DETAILS GROUP BY JOB;
-- Count subscriptions (y) grouped by JOB --
SELECT JOB,count(y) AS "No of Subscribers" FROM BANK_DETAILS GROUP BY JOB;
-- Show JOBs having more than 5 customers --
SELECT JOB,count(*) AS "No of Customers" FROM BANK_DETAILS GROUP BY JOB HAVING count(*)>5;
-- Show JOBs whose average BALANCE is greater than 500 --
SELECT JOB,ROUND(avg(BALANCE),0) AS "BALANCE" FROM BANK_DETAILS GROUP BY JOB HAVING ROUND(avg(BALANCE),0)>500;
-- Show MARITAL groups with at least 10 customers --
SELECT MARITAL,count(*) AS "No of Customers" FROM BANK_DETAILS GROUP BY MARITAL HAVING count(*)>=10;
-- Show JOBs where the maximum BALANCE exceeds 5000--
SELECT JOB,max(BALANCE) FROM BANK_DETAILS GROUP BY JOB HAVING max(BALANCE)>5000;
-- Find the second-highest BALANCE --
SELECT DISTINCT BALANCE FROM BANK_DETAILS ORDER BY BALANCE DESC LIMIT 1 OFFSET 1;
SELECT DISTINCT BALANCE FROM BANK_DETAILS ORDER BY BALANCE DESC LIMIT 1,1;
-- Find all customers who have the maximum BALANCE --
SELECT * FROM BANK_DETAILS WHERE BALANCE=(SELECT MAX(BALANCE) FROM BANK_DETAILS);
-- Find customers older than the average AGE--
SELECT * FROM BANK_DETAILS WHERE AGE>(SELECT AVG(AGE) FROM BANK_DETAILS);
-- Find the percentage of customers with y = 'YES' --
SELECT (SELECT count(*) FROM BANK_DETAILS WHERE y="YES")/(SELECT count(*) FROM BANK_DETAILS)*100 AS "% of YES";
-- Find which JOB has the highest average BALANCE -- 
SELECT JOB,avg(BALANCE) AS "Average Balance" FROM BANK_DETAILS GROUP BY JOB ORDER BY avg(BALANCE) DESC LIMIT 1;
-- Find the average DURATION for customers with y = 'YES' versus y = 'NO' --
SELECT y,avg(DURATION) AS "AVERAGE DURATION" FROM BANK_DETAILS GROUP BY y;

-- CREATE PROCEDURE FOR MAX--
DELIMITER $$
...
-- CREATE PROCEDURE FOR AVERAGE --
DELIMITER $$
CREATE PROCEDURE AVG1()
BEGIN
...
-- CREATE PROCEDURE FOR MINIMUM --
DELIMITER $$
CREATE PROCEDURE MIN1()
BEGIN
...
-- CREATE VIEW TABLE --
CREATE VIEW CUST AS
SELECT AGE,BALANCE,MARITAL FROM BANK_DETAILS;
